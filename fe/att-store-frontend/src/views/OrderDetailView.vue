<template>
  ...
  <script setup>
  import { ref, computed, onMounted } from 'vue'; // Bỏ watch nếu không theo dõi prop đặc biệt
  import { useRouter } from 'vue-router';
  // Import các hàm API thật (cần tạo/định nghĩa trong apiService.js)
  ...
  const columnCount = 7;       // Số cột trong bảng
  const searchPerformed = ref(false); // Để hiển thị đúng thông báo "không tìm thấy"
  ...
  const loadOrders = async () => {
    ...
    try {
      const params = {};
      if (searchTerm.value) {
        // Backend cần hỗ trợ search theo các trường mong muốn (ví dụ: order_code, customer__name,...)
      ...
      if (response.data && Array.isArray(response.data.results)) {
        ...
        orders.value = response.data; // Nếu API trả về mảng trực tiếp
      } else {
      ...
      orders.value = []; // Đảm bảo mảng rỗng khi lỗi
    } finally {
      loading.value = false;
    }
  };
  ...
  const getStatusClass = (status) => `status-${status}`; // Dùng để tạo class CSS
  
  // --- Logic kiểm tra trạng thái nút ---
  ...
  const canComplete = (status) => status === 'processing';
  ...
  const addOrder = () => {
    ...
    // Đảm bảo đã tạo route có name 'create-order'
  ...
  const viewOrder = (orderId) => {
    ...
    // Đảm bảo đã tạo route có name 'order-details'
    router.push({ name: 'order-details', params: { id: orderId } });
  };
  
  const editOrder = (orderId) => {
    console.log(`Sửa đơn hàng ID: ${orderId}`);
     // Đảm bảo đã tạo route có name 'order-edit'
    router.push({ name: 'order-edit', params: { id: orderId } });
  };
  
  // Hàm gọi API Action Duyệt
  const approveOrderAction = async (orderId) => {
      const orderCode = orders.value.find(o=>o.id===orderId)?.order_code || orderId;
      if (!confirm(`Bạn có chắc muốn DUYỆT đơn hàng có mã ${orderCode}?`)) return;
      error.value = null;
      // Không nên set loading ở đây vì nó sẽ làm mờ cả bảng, chỉ nên disable nút
      // Hoặc thêm state loading riêng cho từng dòng/nút
      try {
          await approveOrderAPI(orderId); // Gọi API action
          alert(`Đơn hàng ${orderCode} đã được duyệt thành công!`);
          loadOrders(); // Tải lại danh sách để cập nhật trạng thái
      } catch (err) {
          console.error("Lỗi khi duyệt đơn hàng:", err);
          error.value = `Lỗi duyệt đơn ${orderCode}: ${err.response?.data?.error || err.message || "Lỗi không xác định"}`;
      }
  };
  ...
  const cancelOrderAction = async (orderId) => {
       const orderCode = orders.value.find(o=>o.id===orderId)?.order_code || orderId;
       if (!confirm(`Bạn có chắc muốn HỦY đơn hàng có mã ${orderCode}?`)) return;
       error.value = null;
      try {
          ...
          console.error("Lỗi khi hủy đơn hàng:", err);
           error.value = `Lỗi hủy đơn ${orderCode}: ${err.response?.data?.error || err.message || "Lỗi không xác định"}`;
      }
  };
  
  // Hàm gọi API Action Hoàn tất
  const completeOrderAction = async (orderId) => {
       const orderCode = orders.value.find(o=>o.id===orderId)?.order_code || orderId;
       if (!confirm(`Bạn có chắc muốn đánh dấu HOÀN THÀNH đơn hàng có mã ${orderCode}?`)) return;
       error.value = null;
      try {
          await completeOrderAPI(orderId); // Gọi API action
          alert(`Đơn hàng ${orderCode} đã được hoàn thành!`);
          loadOrders(); // Tải lại danh sách
      } catch (err) {
          console.error("Lỗi khi hoàn thành đơn hàng:", err);
           error.value = `Lỗi hoàn thành đơn ${orderCode}: ${err.response?.data?.error || err.message || "Lỗi không xác định"}`;
      }
  };
  ...
  /* Nút tạo đơn hàng */
  .add-order-button { background-color: #0d6efd; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer; font-weight: 500; transition: background-color 0.2s ease; white-space: nowrap; }
  ...
  /* Màu nút thao tác */
  .btn-view { background-color: #6c757d; color: white;} .btn-view:hover:not(:disabled) { background-color: #5a6268;}
  .btn-edit { background-color: #ffc107; color: #343a40;} .btn-edit:hover:not(:disabled) { background-color: #e0a800;}
  .btn-approve { background-color: #198754; color: white;} .btn-approve:hover:not(:disabled) { background-color: #157347;}
  .btn-cancel { background-color: #dc3545; color: white;} .btn-cancel:hover:not(:disabled) { background-color: #bb2d3b;}
  .btn-complete { background-color: #0dcaf0; color: white;} .btn-complete:hover:not(:disabled) { background-color: #0aa3bd;}
  
  /* Trạng thái */
  .status-badge { padding: 4px 8px; border-radius: 12px; font-size: 0.8rem; font-weight: bold; white-space: nowrap; }
  .status-pending { background-color: #fff3cd; color: #664d03; }
  .status-processing { background-color: #cfe2ff; color: #0a58ca; }
